// --- PATCH: Agar posisi menu tidak kembali otomatis setelah di-drag ---
window.Cheat_Menu = window.Cheat_Menu || {};
window.Cheat_Menu._cheatMenuManualMoved = false;

// --- DRAG & DROP MENU CHEAT (HEADER ONLY, FIXED INIT) ---

// --- DRAG & DROP MENU CHEAT (HEADER ONLY, FIXED, MOVE ALL) ---
(function() {
	function setupDragMenu() {
		if (!window.Cheat_Menu || !window.Cheat_Menu.overlay_box || !window.Cheat_Menu.overlay) return;
		if (document.getElementById('cheat_menu_dragbar')) return;
		var dragBar = document.createElement('div');
		dragBar.id = 'cheat_menu_dragbar';
		...
		dragBar.style.zIndex = '1002';
		dragBar.style.background = 'rgba(0,0,0,0.01)';
		dragBar.style.borderTopLeftRadius = '18px';
		dragBar.style.borderTopRightRadius = '18px';
		window.Cheat_Menu.overlay_box.appendChild(dragBar);

		window.Cheat_Menu._dragging = false;
		window.Cheat_Menu._dragOffsetX = 0;
		window.Cheat_Menu._dragOffsetY = 0;

				function setMenuPosition(x, y) {
					window.Cheat_Menu.overlay_box.style.left = x + 'px';
					window.Cheat_Menu.overlay_box.style.top = y + 'px';
					window.Cheat_Menu.overlay_box.style.right = '';
					window.Cheat_Menu.overlay_box.style.bottom = '';
					window.Cheat_Menu.overlay_box.style.marginLeft = '';
					window.Cheat_Menu.overlay_box.style.marginTop = '';
					window.Cheat_Menu.overlay_box.style.zIndex = '1003';
					// Ikutkan isi menu
					window.Cheat_Menu.overlay.style.left = x + 'px';
					window.Cheat_Menu.overlay.style.top = y + 'px';
					window.Cheat_Menu.overlay.style.right = '';
					window.Cheat_Menu.overlay.style.bottom = '';
					window.Cheat_Menu.overlay.style.marginLeft = '';
					window.Cheat_Menu.overlay.style.marginTop = '';
					window.Cheat_Menu.overlay.style.zIndex = '1004';
					window.Cheat_Menu._cheatMenuManualMoved = true;
					// Simpan posisi ke localStorage
					try {
						localStorage.setItem('cheat_menu_position', JSON.stringify({x:x, y:y}));
					} catch(e) {}
					    // Sesuaikan ukuran background dengan isi
    if (typeof Cheat_Menu.resize_menu_box === "function") {
        Cheat_Menu.resize_menu_box();
    }

				}

		dragBar.addEventListener('mousedown', function(e) {
			if (!window.Cheat_Menu.cheat_menu_open) return;
			window.Cheat_Menu._dragging = true;
			window.Cheat_Menu._dragOffsetX = e.clientX - window.Cheat_Menu.overlay_box.offsetLeft;
			window.Cheat_Menu._dragOffsetY = e.clientY - window.Cheat_Menu.overlay_box.offsetTop;
			document.body.style.userSelect = 'none';
		});
		document.addEventListener('mousemove', function(e) {
			if (window.Cheat_Menu._dragging) {
				let x = e.clientX - window.Cheat_Menu._dragOffsetX;
				let y = e.clientY - window.Cheat_Menu._dragOffsetY;
				setMenuPosition(x, y);
			}
		});
		...
		dragBar.addEventListener('touchstart', function(e) {
			...
			if (e.touches.length === 1) {
				...
				window.Cheat_Menu._dragging = true;
				window.Cheat_Menu._dragOffsetX = touch.clientX - window.Cheat_Menu.overlay_box.offsetLeft;
				window.Cheat_Menu._dragOffsetY = touch.clientY - window.Cheat_Menu.overlay_box.offsetTop;
				document.body.style.userSelect = 'none';
			}
		});
		document.addEventListener('touchmove', function(e) {
			if (window.Cheat_Menu._dragging && e.touches.length === 1) {
				let touch = e.touches[0];
				let x = touch.clientX - window.Cheat_Menu._dragOffsetX;
				let y = touch.clientY - window.Cheat_Menu._dragOffsetY;
				setMenuPosition(x, y);
			}
		});
	...
	// Hook ke open menu agar dragBar selalu ada
		var _openMenu = window.Cheat_Menu && window.Cheat_Menu.update_menu;
		if (_openMenu) {
				window.Cheat_Menu.update_menu = function() {
					// Restore posisi dari localStorage jika ada
					let restored = false;
					try {
						const pos = JSON.parse(localStorage.getItem('cheat_menu_position'));
						if (pos && typeof pos.x === 'number' && typeof pos.y === 'number') {
							setTimeout(function() { setMenuPosition(pos.x, pos.y); }, 0);
							window.Cheat_Menu._cheatMenuManualMoved = true;
							restored = true;
						}
					} catch(e) {}
					if (!window.Cheat_Menu._cheatMenuManualMoved || !restored) {
						_openMenu.apply(this, arguments);
					} else {
						// tetap update isi menu, tapi jangan panggil position_menu
						var oldPositionMenu = window.Cheat_Menu.position_menu;
						window.Cheat_Menu.position_menu = function(){};
						_openMenu.apply(this, arguments);
						window.Cheat_Menu.position_menu = oldPositionMenu;
					}
					setupDragMenu();
				};
		} else {
			// fallback: jalankan setupDragMenu setelah 1 detik jika update_menu belum ada
			setTimeout(setupDragMenu, 1000);
		}
})();
/*:
 * @plugindesc Menu Cheat yang bisa diaktifkan melalui kode VIP dan tombol rahasia. [v1.0]
 * @author Rezky Aimar
 *
 * @help
 * Plugin ini menambahkan menu cheat tersembunyi.
 * Untuk mengaktifkan, pemain harus memasukkan kode VIP.
 * Setelah aktif, tekan tombol "1" untuk membuka menu cheat.
 *
 * - Pastikan file ini dinamai: Cheat_Menu.js
 * - Jangan aktifkan dua plugin Cheat_Menu sekaligus!
 *
 * Tidak ada plugin command.
 */
...
// fungsi baru untuk resize background box
Cheat_Menu.resize_menu_box = function() {
    var height = 20;
    for (var i = 0; i < Cheat_Menu.overlay.children.length; i++) {
        height += Cheat_Menu.overlay.children[i].scrollHeight;
    }
...
Cheat_Menu.append_scroll_selector = function(text, key1, key2, scroll_handler) {
	...
	// Handler agar tidak double trigger di mobile
	let leftHandled = false;
	let rightHandled = false;

	scroll_left_button.addEventListener('touchstart', function(e) {
		if (!leftHandled) {
			leftHandled = true;
			scroll_handler("left", e);
		}
	});
	scroll_left_button.addEventListener('touchend', function() { leftHandled = false; });
	scroll_left_button.addEventListener('click', function(e) {
		if (!('ontouchstart' in window)) {
			scroll_handler("left", e);
		}
	});

	scroll_right_button.addEventListener('touchstart', function(e) {
		if (!rightHandled) {
			rightHandled = true;
			scroll_handler("right", e);
		}
	});
	scroll_right_button.addEventListener('touchend', function() { rightHandled = false; });
	scroll_right_button.addEventListener('click', function(e) {
		if (!('ontouchstart' in window)) {
			scroll_handler("right", e);
		}
	});
...
Cheat_Menu.append_title = function(title) {
	...
	temp.addEventListener('mousedown', function(e) { e.stopPropagation(); e.preventDefault(); });
	temp.addEventListener('touchstart', function(e) { e.stopPropagation(); e.preventDefault(); });
	temp.addEventListener('click', function(e) { e.stopPropagation(); e.preventDefault(); });
	var title_text = title_row.insertCell();
	title_text.className = "cheat_menu_cell_title";
	title_text.innerHTML = title;
	title_text.addEventListener('mousedown', function(e) { e.stopPropagation(); e.preventDefault(); });
	title_text.addEventListener('touchstart', function(e) { e.stopPropagation(); e.preventDefault(); });
	title_text.addEventListener('click', function(e) { e.stopPropagation(); e.preventDefault(); });
...
Cheat_Menu.append_cheat = function(cheat_text, status_text, key, click_handler) {
	...
	cheat.addEventListener('mousedown', function(e) {
		e.preventDefault();
		e.stopPropagation();
		click_handler(e);
	});
	cheat.addEventListener('touchstart', function(e) {
		e.preventDefault();
		e.stopPropagation();
		click_handler(e);
	});
...
Cheat_Menu.update_menu = function() {
    ...
    if (!window.Cheat_Menu._cheatMenuManualMoved) {
...
// HAPUS ATAU KOMENTARI: Event listener touchstart pada overlay agar menu tidak bisa dipindah dengan tap
...
// Pastikan tidak ada event drag/move pada overlay/menu cheat

// Drag & Drop untuk Cheat Menu
Cheat_Menu._dragging = false;
Cheat_Menu._dragOffsetX = 0;
Cheat_Menu._dragOffsetY = 0;

Cheat_Menu.overlay_box.addEventListener('mousedown', function(e) {
	if (!Cheat_Menu.cheat_menu_open) return;
	Cheat_Menu._dragging = true;
	Cheat_Menu._dragOffsetX = e.clientX - Cheat_Menu.overlay_box.offsetLeft;
	Cheat_Menu._dragOffsetY = e.clientY - Cheat_Menu.overlay_box.offsetTop;
	document.body.style.userSelect = 'none';
});
document.addEventListener('mousemove', function(e) {
	if (Cheat_Menu._dragging) {
		let x = e.clientX - Cheat_Menu._dragOffsetX;
		let y = e.clientY - Cheat_Menu._dragOffsetY;
		Cheat_Menu.overlay_box.style.left = x + 'px';
		Cheat_Menu.overlay_box.style.top = y + 'px';
		Cheat_Menu.overlay_box.style.right = '';
		Cheat_Menu.overlay_box.style.bottom = '';
		Cheat_Menu.overlay_box.style.marginLeft = '';
		Cheat_Menu.overlay_box.style.marginTop = '';
...
// Touch support
Cheat_Menu.overlay_box.addEventListener('touchstart', function(e) {
	if (!Cheat_Menu.cheat_menu_open) return;
	if (e.touches.length === 1) {
		let touch = e.touches[0];
		Cheat_Menu._dragging = true;
		Cheat_Menu._dragOffsetX = touch.clientX - Cheat_Menu.overlay_box.offsetLeft;
		Cheat_Menu._dragOffsetY = touch.clientY - Cheat_Menu.overlay_box.offsetTop;
		document.body.style.userSelect = 'none';
	}
});
document.addEventListener('touchmove', function(e) {
	if (Cheat_Menu._dragging && e.touches.length === 1) {
		let touch = e.touches[0];
		let x = touch.clientX - Cheat_Menu._dragOffsetX;
		let y = touch.clientY - Cheat_Menu._dragOffsetY;
		Cheat_Menu.overlay_box.style.left = x + 'px';
		Cheat_Menu.overlay_box.style.top = y + 'px';
		Cheat_Menu.overlay_box.style.right = '';
		Cheat_Menu.overlay_box.style.bottom = '';
...
// Agar cheat menu bisa berpindah/bergeser hanya dengan sentuhan/tap di layar (tanpa tombol 2-9)
// Tambahkan event listener pada overlay untuk sentuhan/tap
// Cheat_Menu.overlay.addEventListener("touchstart", function(event) {
//     // Setiap tap di overlay, pindah menu ke kanan (seperti tombol 2/kanan)
//     if (Cheat_Menu.cheat_menu_open) {
//         Cheat_Menu.position++;
//         if (Cheat_Menu.position > 4) {
//             Cheat_Menu.position = 0;
...
Cheat_Menu.initialize = function() {
	// Jangan aktifkan overlay_openable otomatis di sini!
...
// Fungsi global agar Android bisa mengaktifkan cheat menu setelah kode VIP
window.activateAndShowCheatMenu = function() {
	console.log("Fungsi global aktif!");
	Cheat_Menu.overlay_openable = true;
};
